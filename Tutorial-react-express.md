...
En este artículo voy a mostrar todos los ejercicios del tutorial de [react-express](http://www.react.express/) y voy a analizar todos los ejemplos explicando que significa cada parte de código.
...
- Otra fuente ha sido un tutorial más a nivel [conceptual](https://github.com/reactjs/react-basic)

<br>
Para empezar, es necesario realizar la instalación (o _setup_) de todas las herramientas necesarias para llevar a cabo los ejercicios en tu propio ordenador. Es cierto que actualmente hay muchas opciones de programar, inclusor React, a través de webs específicas sin necesidad de instalaciones previas. Pero para aprender y sentirse cómodo con las tecnologías utilizadas es mucho mejor realizarlo en _local_.

En primer lugar, hay que instalar [_nodejs_](https://nodejs.org/es/). que como dicen en su web es
...
En el tutorial nos dan dos opciones.

1. La más sencilla es utilizar _npm_ para instalar el paquete específico _create-react-app_. Con esta instalación es muy sencillo pasar a programar en un entorno de desarrollo rápidamente creado.
Para instalar _create-react-app_ con _npm_ se tendría que hacer desde consola:
```
npm install -g create-react-app
```
Posteriormente, para crear e inicializar una _app_ React y empezar a programar:
```
...
> Como todos los ejemplos están adaptados a esta segunda opción de instalación, yo he seguido también esta vía. Los ejemplos se podrían haber adaptado a la primera configuración, más sencilla y habitual, pero he querido ser fiel a cómo el tutorial estaba realizado.

> Para seguir el tutorial hay que hacer todo lo que indica en el _setup_ e instalar todas las herramientas ya que, sino, pueden aparecer errores al final al intentar ejecutar el index.js de React final. Por ejemplo, a mi me ha dado problemas porque faltaba instalar los módulos de Babel.
...
Lo primero que aparece es las dos primeras líneas con _imports_:

Esto se hace para importar las dos librearías básicas que nos servirán para trabajar con React:
...
En la segunda variable, _element_ se añade contenido en formato [JSX](http://www.react.express/jsx).
...
La última línea sirve para _renderizar_ el _element_ en el nodo DOM.
```
render(element, node)
```
...
Este ejemplo tan solo cambia del anterior el contenido de la variable _element_.
...
Según la [documentación de React](https://reactjs.org/docs/components-and-props.html), los componentes son:
>Los Componentes te permiten dividir la Interficie de Usuario en piezas independiente y reutilizables, y pensar en cada pieza por ella misma.  
> Conceptualmente son como funciones de Javascript. Aceptan que se le pasen parámetros (que se llamana _props_) y devuelven elementos React describiendo lo que debería aparecer en pantalla.
...
Después se añade un método llamado _render()_ donde va todo el contenido del componente. Ahí se añaden estilos CSS y también el color de fondo se establece como un valor que se le pasará (_prop_). En las clases con los valores que reciben no se utiliza la sintaxis de _props_ sino _this.props_.
